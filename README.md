...
2. سيضيف Vercel المتغير `BLOB_READ_WRITE_TOKEN` تلقائيًا؛ أعد النشر بعد الربط.
3. قسم المطوّر يقبل الملفات العامة حتى 4MB أو رابطًا خارجيًا للملفات الأكبر. لا يُستخدم للمستندات السرية.
...
| `MEMBERS_JSON` | بيانات أعضاء الدخول بصيغة JSON (يفضل PIN مشفّرًا) |
| `MEMBERS_PIN_PEPPER` | مفتاح سري إضافي لتشفير PIN؛ يجب أن يطابق قيمة إنشاء بيانات الأعضاء |
| `BLOB_READ_WRITE_TOKEN` | يضيفه Vercel Blob تلقائيًا لتخزين ملفات المطوّر العامة |
