...
const COMPANY_NAME = "شركة سلطان المالكي للمحاماة والاستشارات القانونية";
let _lite = null;
let _meta = null;
let _tags = null;
let _external = null;
...
export function getExternalDocs() {
  ...
  _external = files.flatMap((file) => {
    ...
    title_en: "",
    summary_ar: String(doc.summary_ar || `مرجع مضاف من ${doc.source_name || "مصدر خارجي"} ضمن مصادر ${COMPANY_NAME}.`).replaceAll("المكتب الرئيسي", COMPANY_NAME),
    ...
    generated_pdf: Boolean(doc.generated_pdf),
    pdf_kind: doc.pdf_kind || (doc.pdf_source ? "source" : ""),
    content_text: doc.content_text || "",
    office: doc.office === "المكتب الرئيسي" ? COMPANY_NAME : (doc.office || COMPANY_NAME),
    external: true,
  }));
  return _external;
}
...
export function getDocsLite() {
  if (!_lite) {
    ...
    const external = getExternalDocs().map((doc) => ({
      ...
      external: true,
      pdf: Boolean(doc.pdf_source || doc.generated_pdf),
      pdfKind: doc.pdf_kind || (doc.pdf_source ? "source" : ""),
      sourcePage: doc.source_page || "",
      searchText: normalizeAr(`${doc.title_ar} ${doc.summary_ar || ""} ${doc.category || ""} ${(doc.tags || []).join(" ")} ${doc.source_name || ""} ${doc.content_text || ""}`),
    }));
    _lite = [...external, ...base];
  }
  return _lite;
...
export function filterDocs({ q, cat, year, valid, ins, tag, mode = "broad", sort = "new", page = 1, per = 24 }) {
  ...
  if (q) {
    ...
    docs = docs.filter((d) => {
      const haystack = d.searchText || normalizeAr(`${d.t || ""} ${d.cat || ""} ${(d.tags || []).join(" ")} ${d.source || ""}`);
      if (mode === "exact") return haystack.includes(needle) || String(d.n || "").includes(raw);
      return terms.length ? terms.every((term) => haystack.includes(term)) : haystack.includes(needle);
    });
  }
